...
import { WRAPPER } from '../config/config';
...
function parseTagToString(tag, value, initialValue, additionalText) {
  ...
  if (deepEqual(value, initialValue)) {
    if (value) {
      text += `${additionalText}${tag}: ${value}\n`;
    }
  } else {
    text += `${WRAPPER} ${additionalText}${tag}: ${value} ${WRAPPER}\n`;
  }
  return text;
}
...
function constructCategoryNote(category, isOwnCategory) {
  ...
  if (isOwnCategory) {
    text += `${WRAPPER} category: ${category.text} ${WRAPPER}\n`;
  } else {
    let field;
    ...
    if (deepEqual(category.value, originalCategory.value)) {
      ...
      text += `${WRAPPER} ${categoryFormatted} ${WRAPPER}\n`;
      for (let i = 0; i < category.fields.length; i += 1) {
        field = category.fields[i];
        if (field.value) {
          text += `${WRAPPER} ${field.key}: ${field.value} ${WRAPPER}\n`;
        }
      }
    }
  }
...
function constructNote() {
  ...
  text += `The modified tags are surrounded with '${WRAPPER}'\n \n`;
